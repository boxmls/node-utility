    ...
    value: function sendError( error, details, callback ) {
      ...
      async.auto({
        ...
        "slack": [ "checkCache", function(done){
          ...
          if( !process.env.SLACK_WEBHOOK_URI || !process.env.SLACK_WEBHOOK_CHANNEL ) {
            return done();
          }
        ...
        "mail": [ "checkCache", function(done){
          ...
          if( !process.env.MANDRILL_KEY_LIVE ) {
            done();
          }

          var mandrillKey = process.env.MANDRILL_KEY_LIVE;
          var mail = module.exports.createMail( {
            "mandrillKey": mandrillKey
          } );
          mail.send( 'admin-alert', {
            "to": [{"email":process.env.ALERT_EMAIL_TO}],
