    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        utility.scrollResults(query, (source, next, body) => {
          ...
          if(error){
            console.error('cache.flushServiceCache: Could not remove [%s] cache for [%s] branch.', colors.green(service), colors.green(branch));
            reject(error);
          }else{
            debug('cache.flushServiceCache: Successfully removed [%s] cache for [%s] branch', colors.green(service), colors.green(branch));
            resolve({ok: true});
          }
        });
      } catch (e) {
